...
# --- Ubidots CONFIG ---
UBIDOTS_URL = "https://industrial.api.ubidots.com/api/v1.6/devices/baarian"
UBIDOTS_TOKEN = "BBUS-UjHRHwATDTiBaDbqb9ZREBg4FpDOxb"  # Ganti dengan token Anda
...
def send_to_ubidots(variable_label, value):
    ...
    payload = {
        variable_label: {"value": value}
    }
    try:
        response = requests.post(UBIDOTS_URL, json=payload, headers=headers)
        if response.status_code in [200, 201]:
            print(f"[Ubidots] Data '{value}' terkirim ke variabel {variable_label}")
        else:
            print(f"[Ubidots] Gagal. Status: {response.status_code}, Respon: {response.text}")
    except Exception as e:
        print("[Ubidots] Error:", str(e))
...
try:
    while cap.isOpened():
        ...
        # Kalimat selesai setelah 5 detik idle + kirim ke Ubidots
        if current_time - last_detection_time >= 5.0 and full_sentence:
            ...
            send_audio_to_mqtt()
            send_to_ubidots("huruf", kalimat)  # ✅ Kirim ke Ubidots
